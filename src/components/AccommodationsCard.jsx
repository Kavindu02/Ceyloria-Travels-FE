...
import { MapPin, Star, ArrowRight, ImageOff, Heart, Bed, CheckCircle, XCircle } from 'lucide-react';
...
// If your accommodations images are in a different bucket (e.g., "accommodations"), change "images" to that name.
const BUCKET_NAME = "images"; 

// --- Helper Function (Copied exactly from TravelCard) ---
const getImageUrl = (imagePath) => {
  ...
  if (!SUPABASE_URL) {
    if (import.meta.env.DEV) console.warn('[AccommodationsCard] VITE_SUPABASE_URL not set; using image value as-is:', targetImage);
    return targetImage;
  }
...
const AccommodationsCard = ({ 
  image,           // Maps to DB: images
  name,            // Maps to DB: name
  location,        // Maps to DB: location
  type,            // Maps to DB: type
  description,     // Maps to DB: description
  pricePerNight,   // Maps to DB: price
  rating,          // Optional
  amenities,       // Optional
  isAvailable,     // Maps to DB: isAvailable
  onDetailsClick,
  id               // Maps to DB: _id
}) => {
  ...
  return (
    <div className="group relative bg-white rounded-[2rem] overflow-hidden shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 border border-gray-100 flex flex-col h-full">
      ...
      <div className="relative h-72 overflow-hidden bg-gray-100">
        ...
        {/* Gradient Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-gray-900/50 to-transparent z-10 opacity-60 group-hover:opacity-40 transition-opacity" />
        ...
        {finalImageSrc && !imgError ? (
          <img 
            ...
            onError={(e) => {
              if (import.meta.env.DEV) console.warn('[AccommodationsCard] image failed to load:', { image, finalImageSrc, currentIdx, errorEvent: e });
        ...
        <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-20">
          ...
          {/* Location Badge */}
          <div className="bg-white/90 backdrop-blur-md text-gray-800 text-xs font-bold px-3 py-2 rounded-full flex items-center gap-2 shadow-sm border border-white/50">
            <MapPin size={14} className="text-blue-600" />
            <span className="uppercase tracking-wide line-clamp-1 max-w-[120px]">{location}</span>
          </div>

          {/* Availability / Rating Badge Group */}
          <div className="flex flex-col gap-2 items-end">
             {/* Availability Badge */}
             <div className={`backdrop-blur-md text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-wider shadow-lg flex items-center gap-1 ${isAvailable ? 'bg-green-500/90' : 'bg-red-500/90'}`}>
                {isAvailable ? <><CheckCircle size={12} /> Available</> : <><XCircle size={12} /> Booked</>}
            </div>

            {/* Rating Star */}
            {rating && (
              <div className="bg-white/90 backdrop-blur-md text-gray-800 text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1 shadow-sm">
        ...
        {type && (
            ...
            <div className="bg-gray-900/80 backdrop-blur-sm text-white px-4 py-2 rounded-xl flex items-center gap-2 text-xs font-bold shadow-lg border border-white/10 capitalize">
        ...
        <button 
          ...
          className={`absolute top-4 right-4 z-20 p-2 rounded-full backdrop-blur-md transition-all ${
            isFavorite ? 'bg-red-500 text-white shadow-red-500/30' : 'bg-white/20 text-white hover:bg-white/30'
          } shadow-lg`}
        >
      ...
      <div className="p-7 flex flex-col flex-grow relative">
        ...
        <p className="text-gray-500 text-sm leading-relaxed mb-6 line-clamp-2">
          {description || "Experience luxury and comfort in this amazing accommodation."}
        </p>
        ...
        {amenities && amenities.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {amenities.slice(0, 3).map((amenity, idx) => (
              <span key={idx} className="text-[10px] font-bold bg-blue-50 text-blue-600 px-3 py-1 rounded-full uppercase tracking-wide">
                {amenity}
              </span>
            ))}
            {amenities.length > 3 && (
        ...
        <div className="mt-auto pt-6 border-t border-gray-100 flex items-center justify-between gap-4">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-1">
              {pricePerNight ? (
                <span className="text-2xl font-black text-gray-900 tracking-tight">${pricePerNight}</span>
              ) : (
